...
function SaveChanges() {
    ...
    data.append('departamento_id',  myJsonString );
    ...
    data.append('genero',   genero ? genero: '' );
    data.append('provincia_id',   provincia_id ? provincia_id: '' );
    data.append('ciudad_id',   ciudad_id ? ciudad_id: '' );
    data.append('telefono',   telefono ? telefono: '' );
    data.append('celular',   celular ? celular: '' );
    data.append('ing_empresa',   ing_empresa ? ing_empresa: '' );
    data.append('modo',   modo ? modo: '' );
    data.append('cargo',   cargo ? cargo: '' );
    data.append('lider',   lider ? lider: '' );
    data.append('direccion',   direccion ? direccion: '' );
    data.append('email',   email ? email: '' );
    data.append('correo_institucional',   correo_institucional ? correo_institucional: '' );
    data.append('tipo_empleado',   tipo_empleado ? tipo_empleado: '' );
...
function EditChanges(identificacion, apellidos, nombres, lider_empleado_id, provincia_id, ciudad_id, direccion, convencional, celular, ing_empresa, genero, email, estado, correo_institucional, cargo_id, tipo_empleado) {
    ...
    $array=Array();
    var objApiRest = new AJAXRest('/uath/DirectorioBandejasUser', {
        identificacion: identificacion
    }, 'post');
    objApiRest.extractDataAjax(function (_resultContent) {
        if (_resultContent.status == 200) {
            $("#departamento_id").select2("val",_resultContent.message).change();
